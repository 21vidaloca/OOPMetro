...
public:
    ...
    // Functionalitate specifica - estimeaza numarul de calatori pe baza timpului de asteptare
    int estimeazaNumarCalatori() const {
        // Cu cat timpul de asteptare e mai mare, cu atat sunt mai multi calatori (ipoteza simplificata)
        return timpAsteptare * 10; // de ex. 30 sec -> 300 calatori
    }
};
...
class Tren {
    ...
    int viteza; // in km/h
    bool esteInMiscare;
    int nrCalatoriActuali;

public:
    Tren() : id("Unknown"), capacitate(0), viteza(0), esteInMiscare(false), nrCalatoriActuali(0) {}

    Tren(const string& id, int capacitate, int viteza)
        : id(id), capacitate(capacitate), viteza(viteza), esteInMiscare(false), nrCalatoriActuali(0) {}

    // Constructor de copiere
    Tren(const Tren& other)
        : id(other.id), capacitate(other.capacitate), viteza(other.viteza),
          esteInMiscare(other.esteInMiscare), nrCalatoriActuali(other.nrCalatoriActuali) {}
    ...
    Tren& operator=(const Tren& other) {
        if (this != &other) {
            ...
            viteza = other.viteza;
            esteInMiscare = other.esteInMiscare;
            nrCalatoriActuali = other.nrCalatoriActuali;
        }
        return *this;
    }
    ...
    friend ostream& operator<<(ostream& os, const Tren& tren) {
        os << "Tren ID: " << tren.id << " | Capacitate: " << tren.capacitate
           ...
           << " | Status: " << (tren.esteInMiscare ? "In miscare" : "Stationar")
           << " | Calatori actuali: " << tren.nrCalatoriActuali;
        return os;
    }
    ...
    bool getEsteInMiscare() const {
        return esteInMiscare;
    }

    // Functionalitate - porneste trenul
    void porneste() {
        esteInMiscare = true;
        cout << "Trenul " << id << " a pornit." << endl;
    }

    // Functionalitate - opreste trenul
    void opreste() {
        esteInMiscare = false;
        cout << "Trenul " << id << " s-a oprit." << endl;
    }

    // Functionalitate - urca calatori
    bool urcaCalatori(int numarCalatori) {
        if (nrCalatoriActuali + numarCalatori <= capacitate) {
            nrCalatoriActuali += numarCalatori;
            return true;
        }
        return false; // Trenul e plin
    }

    // Functionalitate - coboara calatori
    bool coboaraCalatori(int numarCalatori) {
        if (nrCalatoriActuali >= numarCalatori) {
            nrCalatoriActuali -= numarCalatori;
            return true;
        }
        return false; // Nu sunt suficienti calatori
    }

    // Functionalitate - calculeaza gradul de ocupare
    double calculeazaGradOcupare() const {
        return static_cast<double>(nrCalatoriActuali) / capacitate * 100.0;
    }
...
int main() {
    ...
    Tren tren2("M2", 250, 70);
    Tren tren3("M3", 350, 75);
